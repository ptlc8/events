<template>
    ...
    <section>
        <SearchBar :placeholder="$text.get('searchevents')" @click="$router.push({ name: 'search' })" />
    </section>
    <section class="categories">
        <router-link v-for="cat in categories" :to="{ name: 'search', query: { c: cat.id } }" class="categorie">
            <span class="icon">{{ [...$text.get(cat.id)][0] }}</span>
            {{ [...$text.get(cat.id)].slice(1).join('') }}
        </router-link>
    ...
    <section>
        <h2>🎯 {{ $text.get('relevant') }}</h2>
        <div class="events-list">
            <EventPreview vertical class="event" v-for="event in relevantEvents" :event="event" @click="$store.event = event" />
            <RouterLink class="more" :to="{ name: 'search', query: { s: 'relevance' } }">{{ $text.get('view_more') }}</RouterLink>
        </div>
    </section>
    <section>
        <h2>⭐ {{ $text.get('popular') }}</h2>
        <div class="events-list">
            <EventPreview vertical class="event" v-for="event in popularEvents" :event="event" @click="$store.event = event" />
            <RouterLink class="more" :to="{ name: 'search', query: { s: 'popularity' } }">{{ $text.get('view_more') }}</RouterLink>
        </div>
    </section>
    <section>
        <h2>📍 {{ $text.get('nearby') }} ({{ gloc?.name }})</h2>
        <div class="events-list">
            <EventPreview vertical class="event" v-for="event in nearbyEvents" :event="event" @click="$store.event = event" />
            <RouterLink class="more" :to="{ name: 'search', query: { s: 'distance' } }">{{ $text.get('view_more') }}</RouterLink>
        </div>
    </section>
    <section>
        <h2>⏰ {{ $text.get('soon') }}</h2>
        <div class="events-list">
            ...
            <RouterLink class="more" :to="{ name: 'search', query: { s: 'datetime' } }">{{ $text.get('view_more') }}</RouterLink>
        </div>
    </section>
</template>
